...
//    let pixelBuffer:CVPixelBuffer = (UIImage(named: "f2")?.pixelBuffer()!)!
    ...
    let r: Float = min(Float(inputWidth)/Float(imageWidth), Float(inputHeight)/Float(imageHeight))
    let unpad: [Int] = [Int(round(Float(imageWidth)*r)), Int(round(Float(imageHeight)*r))]
    let nw = Float(inputWidth - unpad[0])
    let nh = Float(inputHeight - unpad[1])
    let top = Int(round(nh/2 - 0.1))
    let bottom = Int(round(nh/2 + 0.1))
    let left = Int(round(nw/2 - 0.1))
    let right = Int(round(nw/2 + 0.1))
    padding = [top, bottom, left, right]
    let scaledSize = CGSize(width: unpad[0], height: unpad[1])
...
//      outputCount = try interpreter.output(at: 3)
      all = try interpreter.output(at: 0)
    } catch let error {
      print("Failed to invoke the interpreter with error: \(error.localizedDescription)")
      return nil
    }

    let a: [Float] = [Float](unsafeData: all.data) ?? []
    var boundingBox: [Float] = []
    var cls: [Float] = []
    var scores: [Float] = []
    for i in stride(from: 0, to: a.count, by: 6) {
        boundingBox += a[i...i+3]
        scores.append(a[i+4])
        cls.append(a[i+5])
    }
    
    let resultArray = formatTest(
    ...
    func formatTest(boundingBox: [Float], outputClasses: [Float], outputScores: [Float], outputCount: Int, width: CGFloat, height: CGFloat) -> [Inference]{
        ...
        for i in 0 ..< outSc.count {
            let score = outSc[i]
            let outputClassIndex = Int(outCls[i])
            let outputClass = labels[outputClassIndex + 1]

            var rect: CGRect = CGRect.zero

            // Translates the detected bounding box to CGRect.
            rect.origin.x = CGFloat((outBou[4*i]*Float(inputWidth)-Float(padding[2]))/r)
            rect.origin.y = CGFloat((outBou[4*i+1]*Float(inputHeight)-Float(padding[0]))/r)
            rect.size.width = CGFloat((outBou[4*i+2]*Float(inputWidth))/r)
            rect.size.height = CGFloat((outBou[4*i+3]*Float(inputHeight))/r)
...
//            let newRect = rect.applying(CGAffineTransform(scaleX: width, y: height))
  ...
  private func rgbDataFromBuffer(
    ...
    for _ in 0 ..< padding[0] {
        toppad += [Float](repeating: 114.0/255.0, count: width*3)
    }
    for i in 0..<height {
        floats += [Float](repeating: 114.0/255.0, count: padding[2]*3)
        for j in 0..<width*3 {
            floats.append(Float(bytes[i*width*3 + j])/255.0)
        }
        floats += [Float](repeating: 114.0/255.0, count: padding[3]*3)
    }
