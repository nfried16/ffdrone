...
extension ViewController: CameraFeedManagerDelegate {
    ...
    @objc  func runModel(onPixelBuffer pixelBuffer: CVPixelBuffer) {
        ...
        let curr = Date()
        let currentTimeMs = curr.timeIntervalSince1970 * 1000
        ...
        if(displayResult.inferences.count > 0) {
            var maxConf: Float = 0
            for det in displayResult.inferences {
                maxConf = max(det.confidence, maxConf)
            }
            var currentLoc: CLLocation?
            if(CLLocationManager.authorizationStatus() == .authorizedWhenInUse || CLLocationManager.authorizationStatus() == .authorizedAlways) {
                currentLoc = locationManager.location
            }
            detections.append(Detection(location: currentLoc, img: UIImage(pixelBuffer: pixelBuffer), time: currTime, maxConf: maxConf))
        }
